...
- Rename the sampling entry points to `sample`: `simulate` on `Bicop` / `Vinecop` / `RVineStructure` / `Kde1d` and `utils.simulate_uniform` stay as `DeprecationWarning` aliases, while the unreleased `simulate_conditional` → `sample_conditional` and `_sample_uniform` → `_sample_uniform` hook get none (#297).
...
- Add the `pyvinecopulib.margins` subpackage, starting with `EmpiricalMargin`: the rank transform behind `to_pseudo_obs` as a distribution, so fitting a vine copula to pseudo-observations becomes one configuration of a vine distribution rather than a separate workflow (#292).
...
- `EmpiricalMargin` has no density: `pdf` and `logpdf` raise, and so does a `Vinedist` built on it, naming `Kde1d` as the margin to use instead. The empirical distribution is atomic, so its mass is not a density and would otherwise survive silently into a log-likelihood (#292).
...
- Document vine distributions: a `concepts.rst` section on the `{pdf, cdf, icdf}` margin contract, why `pdf` is the density with respect to the margin's own reference measure, and what the two-step estimator costs (#295).
- Add `examples/11_vine_distributions.ipynb`, worked on data with known margins so every estimate is checked against a truth (#295).
- Drop `AGENTS.md`'s machine-specific "Running heavy commands" section, keeping only the repo-level half (which CI leg covers what). The file's own rule excludes machine-local guidance, and contributors work from machines without those constraints (#295).
