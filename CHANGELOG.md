...
- `DiscretePair` reads a rectangle's probability through the pair's optional `rect_mass` when it advertises one, and by differencing four `cdf` values otherwise. Differencing amplifies any absolute error by `~4/(w1 w2)` in the atom widths, so a `TorchBicop` edge now avoids that floor while a wrapped compiled `Bicop` stays bit-identical to its own quotients (#306).
