...
- `MarginBase.declare(var_type=, support=)` hands a margin what the caller resolved before it is fitted, and the sklearn estimators now supply it for *every* column rather than only those a `margins=` argument leaves unaddressed. `MarginSelector` had been re-inferring the type from the sample and running with no bounds at all on a column whose levels the input declared (#292).
...
- A margin declares `supported_var_types`, the variable types its *family* can serve, where `var_type` reports only what one instance currently is. An explicit candidate that serves the wanted type is configured through `declare` and re-checked, so a family that declares the set but ignores the call is dropped rather than fitted as the wrong kind (#292).
