...
- Add the `pyvinecopulib.margins` subpackage: `Kde1dMargin` (the default; wraps `Kde1d` and handles continuous, discrete and zero-inflated data) and `EmpiricalMargin` (the rank transform behind `to_pseudo_obs`, as a distribution -- its `cdf` reproduces that helper exactly, so fitting a vine copula to pseudo-observations becomes one configuration of a vine distribution rather than a separate workflow). `as_margin` presents a distribution object from another ecosystem as a margin, and `register_margin_adapter` teaches it about one it does not know; SciPy's modern and legacy distributions and `torch.distributions` are recognized out of the box, with autograd surviving the torch adapter and a bisection fallback for the families that declare `icdf` but raise. Coercion is not optional even for objects that already have `pdf` / `cdf` / `icdf`: a modern SciPy *discrete* distribution reports `+inf` for every probability mass, so `as_margin` routes it to `pmf` and derives the left limit by subtracting the mass instead of stepping back a lattice point its `cdf` interpolates across (#292).
- Add the univariate-margin contract to `pyvinecopulib.core`: the generic, `runtime_checkable` protocol `MarginLike` (`pdf` / `cdf` / `icdf`) and its canonical array-backend-agnostic partial implementation `MarginBase`. `pdf` is the density with respect to the margin's *own* reference measure -- a Lebesgue density, a probability mass at an atom, or whichever applies pointwise for a mixed margin -- so the Sklar factorization `log f(x) = log c(F(x)) + sum_j log pdf_j(x_j)` holds unchanged for continuous, discrete and zero-inflated margins with no branch on the variable type. `Kde1d` satisfies the contract directly in all three of its modes. A custom margin need only implement `pdf` / `cdf` and inherits `icdf` (bisection over `support`, widening an unbounded one), `logpdf`, `cdf_left` (the left limit `F(x^-)` a margin with atoms needs, derived from `var_type`), `loglik`, `simulate` (via a `_simulate_uniform` RNG hook) and a structural `__repr__`. Discreteness (`var_type`), the left limit, log-densities, sampling and `support` are optional capabilities rather than protocol members, so distributions from other ecosystems can conform (#292).
...
- Generalize the internal monotone root-finder behind the array-agnostic `hinv1` / `hinv2` so it also inverts a CDF on an unbounded or half-bounded support: bracket endpoints may be arrays (a per-element support) and infinite endpoints are widened outward before bisecting. The unit-interval path the h-inverses use is bit-for-bit unchanged (#292).
...
- Drop `@torch.no_grad()` from the torch backend's cached-grid lookup and from `TorchBicop.hinv1` / `hinv2`. With `cache_integrals=True` the cached `cdf` / `hfunc*` / `hinv*` were gradient-dead, so `TorchVinecop.pdf`'s gradient in `u` was off by 111% of the largest entry on the non-batched path -- while the batched path, which never went through `interp_at`, was right. Both now agree with central differences to 1e-10 (#302).
