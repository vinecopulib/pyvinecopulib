...
- Add the torch half of the vine-distribution stack: `TorchMargin`, a univariate margin on any `torch.distributions` family, and `TorchVinedist`, a `Vinedist` whose copula and margins are all `torch.nn.Module`s -- one `.to(device)` moves the whole joint distribution, `state_dict` captures all of it, and `-dist.logpdf(y).mean().backward()` reaches the margins (#294).
- A `torch.distributions.Distribution` has no `.to()` and contributes nothing to a `state_dict`, so `TorchMargin` registers the *parameters* and rebuilds the distribution on every call -- the shape `TorchBicop` already uses for its grid. `TorchMargin.from_distribution` lifts an existing one (#294).
- `TorchMargin` takes continuous families only, and only those implementing `cdf`: `Gamma` / `Chi2` are rescued by inverting it, while `Beta` / `StudentT` and every discrete family are refused at construction rather than at first evaluation (#294).
- A `Gamma` margin is fittable by its own marginal likelihood but not end to end through a copula, since torch has no derivative for `igamma`'s first argument -- one more argument for keeping the two-step estimator the default (#294).
- `TorchVinedist` insists on torch parts throughout, since a SciPy margin or a compiled `Vinecop` would silently detach every gradient, and adds `log_prob` as the torch-idiomatic spelling of `logpdf`. Its `logpdf` matches the NumPy `Vinedist` to 1e-10 (#294).
...
- Add `pyvinecopulib.core.DiscretePair`, which evaluates a *continuous* pair copula on a discrete or mixed edge from its `pdf` / `cdf` / `hfunc1` / `hfunc2` alone. `BicopLike` stays a two-column continuous contract, so a custom pair copula gains discrete support by implementing `cdf`; the output matches `Bicop`'s own discrete surface bit-for-bit at rotation 0 (#301).
...
- Lift the torch backend's continuous-only restriction on the *copula* half: `TorchVinecop`'s three constructors take `var_types`, the stored pair copulas stay continuous interpolation grids, and an edge that sees an atom is evaluated through `DiscretePair` — so `state_dict` / `.to()` / pickling still see only real `nn.Module` parameters. `TorchBicop.from_data` accepts the four-column layout and takes its ranks from the compiled `to_pseudo_obs(ties_method="random", seeds=[5])`, which is the only thing an atom changes about a TLL fit (`TllBicop::fit` derives its grid from the jittered ranks alone), so the fitted grid matches the C++ one to 1e-14. `TorchVinecopBackend` therefore accepts discrete columns, and the batched fast path declines on a discrete vine rather than evaluating a continuous density (#294).
- Add `TorchKde1d` to `pyvinecopulib.torch`, the torch marginal estimator: fitting delegates to the compiled `Kde1d` and every evaluation is pure torch, on device and under autograd. It is the only torch margin that handles discrete and zero-inflated variables (#294).
- `TorchVinedist.from_data` now fits: a `TorchKde1d` per column, then `TorchVinecop.from_data` on the copula data they produce, so a torch vine distribution lands on one device in one dtype. It refuses covariates, since no torch margin reads them (#294).
- `TorchKde1d.icdf` reproduces the compiled 35-step bisection exactly and then reattaches an exact gradient by one Newton step, so the value is bit-identical to `Kde1d.icdf` while remaining differentiable in the grid (#294).
- The sklearn backend now chooses the *class* of the default margin: `TorchVinecopBackend` fits `TorchKde1d`, so `VineDensity(backend=TorchVinecopBackend())` no longer puts NumPy margins on a torch copula and stops the gradient at the marginal transform (#294).
- `TorchVinedist` accepts a margin with atoms that supplies a left limit, rather than refusing every non-continuous one. `MarginBase` derives one, so what is left to refuse is a margin declaring atoms with no `cdf_left` at all (#294).
