...
- Add the list-of-trees round-trip for structures (upstream's shared `RVineTrees` primitive): `Vinecop.get_trees()` returns the fitted vine as nested `[tree][edge]` lists of `{"conditioned": (a, b), "conditioning": [...], "pair_copula": Bicop}` dicts (carrying the fitted pair copulas), `RVineStructure.get_trees()` returns the bare `(a, b, conditioning)` decomposition, and `RVineStructure.from_trees(d, trees)` is its **faithful** inverse (identity diagonal policy, so `RVineStructure.from_trees(s.dim, s.get_trees()) == s`); `RVineStructure` also gains `__eq__`. The previous `from_trees` selection-finalization behaviour (matching `Vinecop.select`'s matrix) moved to an internal `_from_selected_trees` helper on `RVineStructure`, which the array-agnostic selector uses to stay byte-for-byte with the C++ selector ([vinecopulib#698](https://github.com/vinecopulib/vinecopulib/pull/698)).
