...
- `Kde1d`'s `quantile` method is renamed to `Kde1d.icdf`, matching what both SciPy's modern distribution classes and `torch.distributions` call the inverse CDF, and freeing the name `quantile` of its usual Python meaning of *sample* quantiles. No alias: replace `kde.quantile(p)` with `kde.icdf(p)` (#274).
...
- `VineDensity` and `VineRegressor` take a `margins=` keyword and delegate the whole marginal half to `Vinedist`, so the Sklar assembly has one implementation instead of two. `margins=None` still fits a `Kde1d` per column and reproduces the previous numbers; the internal `_x_kde1d` / `_y_kde1d` attributes are gone (the fitted margins are `distribution_.margins`), and `margins` sits between `backend` and `batch_size`, so a positional call past `backend` shifts (#281).
- `VineRegressor` requires a continuous response margin (#281).
- `VineRegressor` takes its `use_grid=True` quadrature on the probability scale: substituting `p = F_Y(y)` in the conditional expectation cancels the marginal density, so the nodes are now `icdf` of a fixed grid of `n_nodes` probability levels and the weights are the copula density alone. Any continuous margin can be the response, where before the nodes and an extra density factor were read off the response margin's `Kde1d` interpolation grid; the new `n_nodes=401` parameter replaces the implicit dependence on that grid's size. Predictions shift by around 1e-5 response standard deviations for an unbounded response margin, and by far more for a bounded one, whose grid is not uniform in `y` -- the spacing the old weights omitted (#282).

### New features in `pyvinecopulib`

- Add `Vinedist`: a full multivariate distribution built from any vine copula and any univariate margins, so Sklar's theorem becomes an object rather than something callers assemble by hand. `Vinedist.from_data(x, margins=...)` runs the two-step estimator (margins first, then the copula on the transformed data), leaving already-fitted margins alone so fixed and estimated margins can be mixed; `margins=` accepts an alias, one margin, a sequence, a mapping keyed by variable name, or a callable, resolved by `resolve_margins`. `logpdf` sums in log space, and the discrete `(n, d + k)` layout is assembled internally, so `var_types` and the left-limit columns stop being something users hand-build. A `Vinedist` with `margins="empirical"` is the same model as a `Vinecop` fitted to `to_pseudo_obs`, which makes the long-standing copula-only workflow a special case (#278).
- Add parametric margins and family selection to `pyvinecopulib.margins`, behind a new `pyvinecopulib[scipy]` extra: `ParametricMargin` wraps one `scipy.stats` family (`ParametricMargin("gamma", floc=0.0)` pins `loc`; passing the full parameter vector makes a fixed, already-fitted margin), and `MarginSelector` -- itself a margin -- fits every admissible candidate, keeps the winner on `selected_` and forwards evaluation to it, with the per-candidate table on `report_` and `criterion` in `{"aic", "bic", "aicc"}`. `margins="parametric"` reaches it from `resolve_margins` (#279).
    - The candidate set is curated, not "all of SciPy": a blind sweep over the 110 continuous families ranks `vonmises` first on clean gamma data, ahead of the truth by 701 AIC units, because it advertises the whole real line as its support while its density integrates to 63.75 there. Candidates are grouped by support (real line, positive, unit interval, a given `[a, b]`, counts) with `loc` / `scale` pinned per group, and counts are never compared against continuous families.
    - Four checks reject a fit -- finite parameters, no parameter at a bound or past a degeneracy threshold, a support containing the data, a finite log-likelihood -- and every rejection gets a row in `report_` with its reason rather than being skipped. When nothing is admissible the fallback is `Kde1dMargin` plus one `UserWarning`, never `norm`.
    - `n_parameters` counts only freely estimated parameters. SciPy's legacy fitter always returns `loc` and `scale`, so the length of the parameter vector overstates it whenever one was pinned, by enough to reverse a close comparison. Continuous families use that legacy fitter rather than `scipy.stats.fit`, which pins `loc = 0` / `scale = 1` by default and is far slower; the discrete families have no legacy fitter and go through `scipy.stats.fit` with explicit bounds.
...
- Add the torch half of the vine-distribution stack: `TorchMargin`, a univariate margin on any `torch.distributions` family, and `TorchVinedist`, a `Vinedist` whose copula and margins are all `torch.nn.Module`s -- so one `.to(device)` moves the whole joint distribution, `state_dict` captures all of it, and `-dist.logpdf(x).mean().backward()` reaches the marginal parameters. A `torch.distributions.Distribution` has no `.to()` and contributes nothing to a `state_dict`, so `TorchMargin` registers the *parameters* and rebuilds the distribution on every call, the shape `TorchBicop` already uses for its grid; `TorchMargin.from_distribution` lifts an existing one. Continuous families only, and only those implementing `cdf`: `Gamma` / `Chi2` are rescued by inverting it, while `Beta` / `StudentT` (which have no `cdf` to invert) and every discrete family are refused at construction rather than at first evaluation. Because `torch` has no derivative for `igamma`'s first argument, a `Gamma` margin is fittable by its own marginal likelihood but not end to end through a copula -- one more argument for keeping the two-step estimator the default. `TorchVinedist` insists on torch parts throughout, since a SciPy margin or a compiled `Vinecop` would silently detach every gradient, and adds `log_prob` as the torch-idiomatic spelling of `logpdf`. Its `logpdf` matches the NumPy `Vinedist` on the same fitted model to 1e-10, the tolerance the torch-vs-C++ copula cascade already pins (#283).
- Generalize the internal monotone root-finder behind the array-agnostic `hinv1` / `hinv2` so it also inverts a CDF on an unbounded or half-bounded support: bracket endpoints may be arrays (a per-element support) and infinite endpoints are widened outward before bisecting. The unit-interval path the h-inverses use is bit-for-bit unchanged (#275).
- Add `Kde1d.is_fitted`, so callers can tell a constructed estimator from a fitted one without reaching into `grid_points` or catching the error `pdf` raises (#274).
...
- Teach the array-agnostic `VinecopBase` cascades the discrete data layouts, so a vine with atoms is no longer the compiled `Vinecop`'s privilege: `_bind_vine(..., var_types=...)` declares which variables are discrete, `pdf` / `rosenblatt` / `cdf` accept the expanded `(n, 2d)` and compact `(n, d + k)` layouts, and every tree level carries a parallel left-limit cascade so each pair copula receives the four-column `[u1, u2, u1^-, u2^-]` input. `pair_var_types(tree, edge)` reports the types a hosted pair must declare, `rosenblatt` gains `randomize_discrete` / `seeds` matching `Vinecop.rosenblatt`, and the batched fast path declines on a discrete vine rather than silently evaluating a continuous density. Given the same pair copulas the output is bit-for-bit identical to `Vinecop`'s on all three cascades; `Vinedist` on a discrete margin now runs through it (#280).
...
- The sklearn estimators publish the fitted model as `distribution_`, a `Vinedist` that evaluates through the fitted backend, and the family-selection table of any selecting margin as `selection_report_` (#281).
- Add `Vinedist.copula_data` and `Vinedist.copula_var_types`: from a set of margins, the copula-scale layout for some data and the `var_types` a copula must be fitted with. That is what "fit your own copula, then wrap it" needs, and what the sklearn estimators use (#281).
- `resolve_margins` takes `default=`, one specification per variable, so a caller that knows something per variable keeps it for the variables a mapping does not address (#281).
...
- Fit the `{0, 1}` dummies of an expanded unordered categorical on `[0, 1]` in the sklearn estimators. Their bounds are known exactly, but were never passed, so the kernel-density grid was padded past the data and put mass on values that cannot occur (#281).
- Pass the declared support of an ordered categorical to its marginal fit in the sklearn estimators. Only the expanded `{0, 1}` dummies were bounded, so a count column's kernel-density grid was still padded below its smallest level: on Poisson(3) counts 0.4% of the fitted mass sat at or below `-1`, a value the variable cannot take (#290).
- `Vinedist.cdf` hands the copula the full `(n, d + k)` layout. A distribution function needs only the marginal `F` values and the copula reads only those, but it validates the whole layout, so a margin with atoms made `cdf` raise (#281).
- Reject complex `X` / `y` in the sklearn estimators rather than casting each column to float and dropping the imaginary part in silence (#281).
