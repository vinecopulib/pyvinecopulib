...
- Compute the `tll` fit's moving-average window smoother in `O(n)` from prefix sums rather than convolving against a `2 * ceil(n / 5) + 1`-tap kernel, which made it `O(n**2)`: the window grows with the data, so at n = 12384 it was 4955 taps and 97% of a whole vine fit. A d = 9 fixed-structure fit on cuda at float64 goes 20.9 s to 0.91 s, from 0.50x to 11.6x single-threaded C++ and 4.5x 16-thread C++; the fitted grid still matches C++ inside the 1e-11 gate, and is bit-identical on 4 of 9 cases measured. `tools_stats::win` reaches the same box mean by FFT, which is also `O(n log n)` and would have fixed it; prefix sums additionally batch over a leading axis and need no cached plan (#315).
