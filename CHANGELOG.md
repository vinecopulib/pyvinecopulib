...
- The sklearn backend now chooses the *class* of the default margin: `TorchVinecopBackend` fits `TorchKde1d`, so `VineDensity(backend=TorchVinecopBackend())` no longer puts NumPy margins on a torch copula and stops the gradient at the marginal transform (#302).
...
- Drop `@torch.no_grad()` from `InterpolationGrid2D.interp_at` and from `TorchBicop._hinv_raw` / `hinv1` / `hinv2`. With `cache_integrals=True` the cached `cdf` / `hfunc*` / `hinv*` were gradient-dead, so `TorchVinecop.pdf`'s gradient in `u` was off by 111% of the largest entry on the non-batched path -- while the batched path, which never went through `interp_at`, was right. Both now agree with central differences to 1e-10 (#302).
