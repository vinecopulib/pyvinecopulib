...
- Port the vinecopulib#691 bucket-accelerated cell search to the torch backend: `_batched_cell_index` can resolve the grid cell by an O(1) bucket lookup (`_build_cell_lookup`) plus a fixed guarded advance instead of `torch.searchsorted`, index-identical to the previous path (parity preserved bit-for-bit). It is **off by default** — on CPU it is 2–4× faster on the isolated cell search but mixed end-to-end (it speeds up `pdf` yet regresses the integration-heavy `hfunc` path), so `InterpolationGrid2D` / `BatchedVine` only build and use the table when `PYVINECOPULIB_TORCH_CELL_LOOKUP=1`, pending a GPU verdict. Benchmarkable via `scripts/bench_torch_bicop_fit.py` (`--mode cellidx` for the isolated primitive; `--mode eval` / `--mode hinv` with the env var set for end-to-end).
