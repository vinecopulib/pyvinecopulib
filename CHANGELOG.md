...
- Guard the per-entry structure accessors (`RVineStructure.struct_array` / `min_array` / `needed_hfunc1` / `needed_hfunc2`) against a tree above `trunc_lvl`. Upstream indexes the triangular array without a bounds check, so a truncated structure **segfaulted** the interpreter; it now raises (#306).
