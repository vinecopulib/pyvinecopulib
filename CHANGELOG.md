...
- `TorchKde1d.icdf` reproduces the compiled 35-step bisection exactly and then reattaches an exact gradient by one Newton step, so the value is bit-identical to `Kde1d.icdf` while remaining differentiable in both `p` and the grid (#302).
