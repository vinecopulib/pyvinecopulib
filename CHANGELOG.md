...
- Add `BatchedVineEnsemble` to `pyvinecopulib.torch`: one batched cascade over a set of fitted `TorchVinecop`s sharing a dimension, a truncation level and a grid, with `pdf` returning `(M, n)` and `rosenblatt` returning `(M, n, d)`. Per evaluation it fires `trunc_lvl` stacked calls rather than `M * trunc_lvl`, and the set is one `torch.compile` code object, so the `cache_size_limit` cliff that leaves a looped ensemble with eight compiled members and the rest silently eager does not apply. It regroups nothing, so it agrees with the per-vine loop to the last bit -- exactly on x86-64, within 1 ULP on arm64 and Windows, where an elementwise kernel may vectorize a stacked tensor differently. A set disagreeing on the grid, the truncation or the cache setting is refused rather than mis-stacked (#310).
