...
- Remove `utils.benchmark`, a timing harness with no caller or test, and three internals with no reader: the `supported_var_types` declaration on margins, the `method` field on `FitControlsTorchBicop` (a one-value enum nothing dispatched on), and the `Kde1d` margin adapter (a provable no-op, since the structural `MarginLike` check passes it through unchanged) (#326).
