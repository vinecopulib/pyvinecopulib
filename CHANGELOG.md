...
- Refuse a one-dimensional `x` with a message that names both readings: the shape says nothing about which axis is which, so `prepare_covariates` points at a reshape to `(n, 1)` for one covariate per observation, and at `covariate_row(x)` for one row shared across them (#334).
