...
- Reorganize the public API into the `core` / `families` / `utils` / `sklearn` subpackages (#207).
    ...
    - `repr` and pickle now use canonical module paths (`Bicop.__module__ == "pyvinecopulib.core"`, `Kde1d.__module__ == "pyvinecopulib.core"`, etc.); pickles from releases up to 0.7.x still load via the deprecated aliases.
...
- `Kde1d`'s `quantile` is renamed to `Kde1d.icdf`, the name both modern SciPy and `torch.distributions` use for the inverse distribution function, and one that does not read as *sample* quantiles. No alias: replace `kde.quantile(p)` with `kde.icdf(p)` (#292).
- `Kde1d` moves from `pyvinecopulib.utils` to `pyvinecopulib.core`, where it sits beside the margin contract it now satisfies. `utils.Kde1d` never shipped in a release, so it is gone rather than deprecated; the released top-level `pyvinecopulib.Kde1d` still resolves and still warns (#292).
- `Kde1d` is a `MarginLike` in its own right, the way `Bicop` and `Vinecop` satisfy their contracts, and gains `var_type` / `support` / `cdf_left` / `logpdf` / `n_parameters` / `family_name` plus a `fit` that returns `self` (#292).
- `Kde1d.loglik` is a method taking optional data rather than a property, matching `Bicop.loglik` and `Vinecop.loglik`: `loglik()` returns the value attained at the fit and `loglik(x)` evaluates given data. Replace `kde.loglik` with `kde.loglik()` (#292).
...
- `VineDensity` and `VineRegressor` take a `margins=` keyword and delegate the whole marginal half to `Vinedist`, so the Sklar assembly has one implementation instead of two. `margins=None` still fits a `Kde1d` per column and reproduces the previous numbers (#292).
- The sklearn estimators' internal `_x_kde1d` / `_y_kde1d` attributes are gone -- the fitted margins are `distribution_.margins` -- and `margins` sits between `backend` and `batch_size`, so a positional call past `backend` shifts (#292).
- `VineDensity` refuses a margin with no density at fit time rather than at the first `score_samples`. `VineRegressor` accepts one: its quadrature reads the copula density and the response margin's `icdf` only (#292).
- `VineRegressor` requires a continuous response margin (#292).
- `VineRegressor` takes its `use_grid=True` quadrature on the probability scale: substituting `p = F_Y(y)` in the conditional expectation cancels the marginal density, so the nodes are `icdf` of a fixed grid of `n_nodes` probability levels and the weights are the copula density alone (#292).
- Any continuous margin can now be `VineRegressor`'s response, where before the nodes and an extra density factor were read off the response margin's `Kde1d` interpolation grid; `n_nodes=401` replaces the implicit dependence on that grid's size (#292).
- `VineRegressor` predictions shift by around 1e-5 response standard deviations for an unbounded response margin, and by far more for a bounded one, whose grid is not uniform in `y` -- the spacing the old weights omitted (#292).
- `VineRegressor` refuses a conditional response margin: the quadrature inverts one shared probability grid, which a margin whose quantiles move with the covariates turns into one grid per test row (#292).
- Remove the `batched` field from `FitControlsTorchVinecop`. Its one consumer, `TorchVinecopBackend.pdf`, passed the field's `False` default and so pinned the sklearn torch backend to the per-edge cascade on CUDA, where every other call on the same vine already resolved `batched` per device; nothing passes it now, and it stays overridable per call (#307).
- Drop Python 3.10, which reaches end of life in October 2026: `requires-python` is now `>=3.11`, the wheel matrix is 10 wheels rather than 15, and the `[scipy]` extra floors at `scipy>=1.16`, the first scipy release to require 3.11 (#292).

- Bump `lib/vinecopulib` to main (Chatterjee's xi, [vinecopulib#754](https://github.com/vinecopulib/vinecopulib/pull/754), and the bounds-checked per-entry structure accessors, [vinecopulib#756](https://github.com/vinecopulib/vinecopulib/pull/756)) and `lib/wdm` to v0.3.0, and port the two `tll` behavior changes the torch evaluator mirrors: the margin renormalization is the geometric mean of both sweep orders iterated to `1e-10` (`norm_times = 3` -> `norm_maxiter = 25`, renamed because the meaning changed from "exactly" to "at most"), and `hfunc` / `hinv` no longer floor the interpolated density at `1e-4`. Every `tll` fit moves, continuous and discrete alike, and `tll` is the default family (#305).
- `cache_integrals=True` changes meaning: the cached `cdf` / `hfunc*` are now exact rather than a bilinear surrogate with ~1e-3 mean error, and they carry an exact gradient in `values` as well as in `u`. `hinv*` no longer read a cache in either mode -- locating the bracketing cell needs the conditional cumulative along the whole free axis -- so they were off by up to 2e-2 and are now bit-identical to the on-the-fly path (#305; #307 reads that cumulative off the prefix table, so the two modes agree to rounding rather than exactly).
### New features in `pyvinecopulib`

- Add `FitControlsTorchBicop.compile_fit`, which fuses the `tll` bandwidth search's per-pass body with `torch.compile`. An ACE pass is 39 kernel launches over tensors small enough that the arithmetic never dominates, so fusing it is worth 2.0x on a pair fit at `n = 500`, 1.4x at 10000, and 1.04x at 30000 -- never a loss, but off by default because the first call compiles for seconds and only a caller fitting many pairs in one process earns that back (#319).
- Fit a whole tree level in one call on the torch path. `VinecopBase.fit` takes an optional `fit_level` hook (`FitLevel`) that it prefers over `fit_edge` for a level whose edges are all continuous and unconditional, gathering the level's inputs before fitting any of it; `TorchBicop.from_data_batched` fits `P` pairs from one `(P, n, 2)` sample, sharing the grid and advancing every lane's bandwidth search together with a freeze as each converges. `FitControlsTorchVinecop.batched_fit` resolves per device like the cascade's `batched`. On cuda a whole vine fit is 1.7-4.0x faster along a given structure and 1.3-2.0x when it selects one, over `d` in 5-20 and `n` in 2000-12000, and never slower than fitting edge at a time; the result agrees with the per-edge fit to floating point on every device, and the selected structure matches exactly because the tree criterion reads ranks rather than last bits (#316).
- Size the `tll` kernel evaluation's grid block from a memory budget rather than fixing it at 256 points. Six `(P, block, n)` temporaries are live at once, so a fixed block grew the fit's peak with both the sample and the level width: a `d = 25`, `n = 20000` vine held 5.7 GiB, on cards that commonly have 8. Blocking the grid axis is exact -- grid points do not interact -- so shrinking the block trades launches for memory and nothing else. Peak now stays near 280 MiB across `d` in 9-25 and `n` in 12000-20000, an 18x reduction at the largest, with fits measuring slightly faster rather than slower (#316).
...
- Evaluate the batched TLL cascade from the exact prefix tables (torch): with `cache_integrals=True` the per-tree-level batched `hfunc1` / `hfunc2` reconstruct the conditional distribution function in closed form from one `(N, m, m)` cumulative table per argument, replacing the stacked bilinear value caches. Measured on an RTX 5070 (`scripts/bench_torch_vinecop.py`): on CUDA the batched fast path runs 2.8-12x faster than the non-batched cascade (`pdf` / `rosenblatt`, d = 10-20, n = 2000-10000 -- ~3x at d = 10, 6-12x at d = 20; `pdf` at d = 20, n = 10000 is 234 -> 34 ms) and 1.0-10.8x faster than single-threaded C++. Against C++ on 16 threads it is 0.25-1.11x, i.e. usually slower, so CUDA is a win over a *serial* baseline rather than over a threaded one at these sizes. `inverse_rosenblatt` is never batched, so `sample` and `cdf` do not benefit at all (0.04-0.82x vs 16-thread C++) (#305; both superseded by #307).
- Record what the torch backend costs to *fit*, which no benchmark covered and no note mentioned: `TorchVinecop.from_data` is 2.7-6.9x slower than single-threaded C++ on cuda and 11-45x slower on cpu (d = 5-20, n = 1000-5000; e.g. d = 10, n = 5000: C++ 2.1 s, cuda 5.8 s, torch cpu 93 s). Structure selection adds almost nothing on top of the pair fits -- `select` and `fit` time within a few percent of each other -- so the cost is the per-pair `tll` fit, where a warm profile puts 67% of a cuda fit in `.item()` calls inside `_ace`'s two convergence loops. The torch backend is for evaluating and differentiating a vine, not for fitting one quickly (#305).
- Add `Vinedist`: a multivariate distribution from any vine copula and any margins, so Sklar's theorem is an object rather than something callers assemble. `Vinedist.from_data(y, margins=...)` runs the two-step estimator, leaves already-fitted margins alone, and builds the discrete `(n, d + k)` layout itself (#292).
- Add parametric margins and family selection to `pyvinecopulib.margins`, behind a new `pyvinecopulib[scipy]` extra: `ParametricMargin` wraps one `scipy.stats` family, and `MarginSelector` -- itself a margin -- fits every admissible candidate, keeps the winner on `selected_` and reports the rest (#292).
    - The candidate set is curated, not "all of SciPy": a blind sweep over the 110 continuous families ranks `vonmises` ahead of the truth on clean gamma data, because it advertises the whole real line while its density integrates to 63.75 there. Candidates are grouped by support with `loc` / `scale` pinned per group, and counts are never compared against continuous families.
    - Four checks reject a fit -- finite parameters, none at a bound or past a degeneracy threshold, a support containing the data, a finite log-likelihood -- and every rejection gets a row in `report_` with its reason. When nothing is admissible the fallback is `Kde1d` plus one `UserWarning`, never `norm`.
    - `n_parameters` counts only freely estimated parameters: SciPy's legacy fitter always returns `loc` and `scale`, so the parameter vector's length overstates it whenever one was pinned, by enough to reverse a close comparison. Continuous families use that legacy fitter; the discrete ones have none and go through `scipy.stats.fit` with explicit bounds.
    - Neither supports observation weights: nothing in SciPy's estimation surface accepts them, so `fit(x, weights=...)` raises instead of silently returning an unweighted fit.
- Add OpenTURNS margins behind a `pyvinecopulib[openturns]` extra: `OpenTURNSMargin` wraps one distribution or fits one family through a `DistributionFactory`, and `OpenTURNSSelector` runs the criterion loop -- ours, since `FittingTest.BestModelBIC` is order-dependent (#292).
- The OpenTURNS extra is installed on every CI leg except macOS: its single `cp39-abi3` wheel carries its own OpenMP runtime, and co-resident with torch's the test process dies with `SIGSEGV` inside `int_on_grid_batched` on every arm64 Python measured (#292).
- Add the `pyvinecopulib.margins` subpackage: the univariate half of a vine distribution, so fitting a vine copula to pseudo-observations becomes one configuration of a vine distribution rather than a separate workflow (#292).
- Add `as_margin`, which presents a distribution object from another ecosystem as a margin, and `register_margin_adapter` for one it does not know. SciPy's modern and legacy distributions and `torch.distributions` are recognized out of the box, with autograd surviving the torch adapter (#292).
- Coercion is not optional even for an object that already has `pdf` / `cdf` / `icdf`: a modern SciPy *discrete* distribution reports `+inf` for every probability mass, so `as_margin` routes it to `pmf` and subtracts the mass for the left limit, its `cdf` being an interpolant off the lattice (#292).
- Add the univariate-margin contract to `pyvinecopulib.core`: the generic, `runtime_checkable` protocol `MarginLike` (`pdf` / `cdf` / `icdf`) and its canonical array-agnostic partial implementation `MarginBase`, which a custom margin subclasses by defining `pdf` and `cdf` alone (#292).
- Every margin member takes the observations positionally and optional exogenous covariates as a keyword-only `x`, mirroring `BicopLike`, so a margin may be conditional. `Vinedist` forwards `x` to every margin declaring `supports_covariates` and to a copula declaring it too (#292).
- `MarginBase.loglik(y=None)` follows `Bicop.loglik`: called without data it returns the value attained at the fit, with data it evaluates one. There is no second `fitted_loglik` name (#292).
- Fitting refuses covariates a margin cannot read, where evaluation ignores them: one distribution may mix conditional and unconditional margins, but silently estimating `f(y)` when `f(y | x)` was asked for is a different model (#292).
- Covariates that nothing reads are refused rather than ignored: `Vinedist.from_data(y, x=...)` raises when every margin specification is unconditional, and evaluation raises when neither the margins nor the copula declare `supports_covariates` (#292).
- `MarginBase.declare(var_type=, support=)` hands a margin what the caller resolved before it is fitted, and the sklearn estimators supply it for every column rather than only those a `margins=` argument leaves unaddressed -- `MarginSelector` had been re-inferring both from the sample (#292).
- `MarginSelector` raises when no candidate is admissible, instead of substituting a kernel-density margin: answering a parametric request nonparametrically is a silent downgrade, and one `UserWarning` is easy to lose in a loop over fifty columns. Pass `on_failure="fallback"` for the old behavior (#292).
- A margin declares `supported_var_types`, the types its *family* can serve, where `var_type` reports only what one instance is. A candidate serving the wanted type is configured through `declare` and re-checked, so one that declares the set but ignores the call is dropped rather than fitted as the wrong kind (#292).
- Candidates that share a family *and* its pinned parameters are deduplicated: two such fits tie exactly on every criterion, making the winner an artifact of iteration order and duplicating the report's rows (#292).
- Add `Vinedist.margin_summary()` and the estimators' `margin_summary_`: one row per variable naming the margin, its family, variable type, support, parameter count and log-likelihood. Where `selection_report` covers only the margins that chose a family, this covers every variable (#292).
- A margin that declares atoms and supplies no `cdf_left` now gets the derived left limit rather than a copy of its `cdf`. Copying it left the pair copula a zero-width rectangle, so the marginal mass stopped canceling and the joint density integrated to 1.08 rather than 1.0, silently (#292).
- `MarginSelector` rejects a continuous family whose `scale` collapses onto an atom. A density concentrating on a repeated value diverges, so on data that is half exact zeros a `t` with `scale` around 1e-20 beat every honest candidate by more than 12,000 AIC units (#292).
- An explicit `MarginSelector(candidates=...)` list is partitioned by kind, as `"auto"` already was, and says what it dropped: a probability mass and a Lebesgue density are not comparable on one information criterion, and the density won on count data for most seeds (#292).
- `MarginBase.icdf` bisects 110 times rather than 50, and snaps to the lattice a `"d"` margin declares. 50 steps resolve the unit interval but collapsed every small quantile on an unbounded support onto one value, and an answer a few ulp below an integer was one its own `cdf_left` rejected (#292).
- `Vinedist.marginal_cdf` / `marginal_icdf` build their result in the margins' array namespace rather than the input's, so a torch copula hosting NumPy margins returns a NumPy result instead of raising (#292).
- `Vinedist.from_data` no longer writes `weights` into the caller's `FitControlsVinecop`, which had silently weighted every later fit that reused it (#292).
- `MarginLike.pdf` is the density with respect to the margin's *own* reference measure -- Lebesgue, or a probability mass at an atom -- so `log f(y) = log c(F(y)) + sum_j log pdf_j(y_j)` holds for continuous, discrete and zero-inflated margins alike, with no branch on the variable type (#292).
- Discreteness (`var_type`), the left limit `F(y^-)`, `logpdf`, `sample` and `support` are optional capabilities rather than protocol members, so distributions from other ecosystems conform; `MarginBase` derives each one (#292).
- Generalize the internal monotone root-finder behind the array-agnostic `hinv1` / `hinv2` so it also inverts a cdf on an unbounded or half-bounded support: bracket endpoints may be arrays, and infinite ones are widened outward before bisecting. The unit-interval path is bit-for-bit unchanged (#292).
- Add the torch half of the vine-distribution stack: `TorchMargin`, a univariate margin on any `torch.distributions` family, and `TorchVinedist`, a `Vinedist` whose copula and margins are all `torch.nn.Module`s -- one `.to(device)` moves the whole joint distribution, `state_dict` captures all of it, and `-dist.logpdf(y).mean().backward()` reaches the margins (#302).
- A `torch.distributions.Distribution` has no `.to()` and contributes nothing to a `state_dict`, so `TorchMargin` registers the *parameters* and rebuilds the distribution on every call -- the shape `TorchBicop` already uses for its grid. `TorchMargin.from_distribution` lifts an existing one (#302).
- `TorchMargin` takes continuous families only, and only those implementing `cdf`: `Gamma` / `Chi2` are rescued by inverting it, while `Beta` / `StudentT` and every discrete family are refused at construction rather than at first evaluation (#302).
- A `Gamma` margin is fittable by its own marginal likelihood but not end to end through a copula, since torch has no derivative for `igamma`'s first argument -- one more argument for keeping the two-step estimator the default (#302).
- `TorchVinedist` insists on torch parts throughout, since a SciPy margin or a compiled `Vinecop` would silently detach every gradient, and adds `log_prob` as the torch-idiomatic spelling of `logpdf`. Its `logpdf` matches the NumPy `Vinedist` to 1e-10 (#302).
- Add `Kde1d.is_fitted`, so callers can tell a constructed estimator from a fitted one without reaching into `grid_points` or catching the error `pdf` raises (#292).
- Teach the array-agnostic `VinecopBase` cascades the discrete data layouts, so a vine with atoms is no longer the compiled `Vinecop`'s privilege: `pdf` / `rosenblatt` / `cdf` accept the expanded `(n, 2d)` and compact `(n, d + k)` layouts, and every tree level carries a parallel left-limit cascade (#292).
- `pair_var_types(tree, edge)` reports the types a hosted pair must declare, `VinecopBase.rosenblatt` gains `randomize_discrete` / `seeds` matching `Vinecop.rosenblatt`, and the batched fast path declines on a discrete vine rather than silently evaluating a continuous density (#292).
- Teach the array-agnostic fit engines about discrete variables: `VinecopBase.fit` and `VinecopBase.select` take `var_types`, carry the same left-limit cascade the evaluators do, and hand each edge's types to `fit_edge` as a keyword — only on the edges that have one, so a callback written for a continuous vine keeps working. Edge weights read the value columns only, so the selected R-vine matrix stays byte-for-byte the compiled selector's. `FitEdge` widens to `Callable[..., BicopLike]` (#301).
- Add `pyvinecopulib.core.DiscretePair`, which evaluates a *continuous* pair copula on a discrete or mixed edge from its `pdf` / `cdf` / `hfunc1` / `hfunc2` alone. `BicopLike` stays a two-column continuous contract, so a custom pair copula gains discrete support by implementing `cdf`; the output matches `Bicop`'s own discrete surface bit-for-bit for every family, and satisfies `sum_atoms c * (u1 - u1^-) == 1` exactly (#301).
- Add conditional sampling to the array-agnostic `VinecopBase`: `sample_conditional(u_cond, *, qrng=, num_threads=, seeds=, conditioning_set=, x=)` holds a subset of the variables fixed and draws the rest, `reorient(conditioning_set)` returns the value-preserving relabeling that puts a chosen subset at the order tail (returned rather than applied in place, since the base class does not own pair storage), `conditioning_set` on `rosenblatt` / `inverse_rosenblatt` evaluates through a reoriented view without mutating, and `select` takes `conditioning_set` as well — reachable from torch through `FitControlsTorchVinecop.conditioning_set`. Hosting a `Vinecop`'s own pair copulas and RNG, the sampler reproduces `Vinecop.sample_conditional` bit for bit, and conditioning-aware selection reproduces the compiled selector's R-vine matrix exactly. Discrete conditioners are supported (both `u_cond` layouts) and reproduced up to their atom. A relabeling is refused on a non-simplified vine, where it would permute the columns of each pair copula's conditioning matrix; conditioning on the order tail needs none and is always allowed (#301).
- Add `Vinedist.sample_conditional(y_cond, *, conditioning_set=, x=)`, the data-scale counterpart: conditioning values go in on the original scale and a discrete conditioner needs no left-limit column, since it is derived from that variable's margin (#301).
- Add `TorchKde1d` to `pyvinecopulib.torch`, the torch marginal estimator: fitting delegates to the compiled `Kde1d` and every evaluation is pure torch, on device and under autograd. It is the only torch margin that handles discrete and zero-inflated variables (#302).
- `TorchVinedist.from_data` now fits: a `TorchKde1d` per column, then `TorchVinecop.from_data` on the copula data they produce, so a torch vine distribution lands on one device in one dtype. It refuses covariates, since no torch margin reads them (#302).
- `TorchKde1d.icdf` reproduces the compiled quantile step for step and then reattaches an exact gradient by one Newton step, so the correction does not move the value while it stays differentiable in both `p` and the grid (#302, #312).
- The sklearn backend now chooses both the *class* of the default margin and how the fitted parts are assembled, through `default_margin` and `bind_distribution`. `TorchVinecopBackend` fits `TorchKde1d` margins on its controls' device and dtype, and publishes `distribution_` as a `TorchVinedist` over the raw `TorchVinecop` -- so `VineDensity(backend=TorchVinecopBackend()).distribution_` moves with `.to(device)` and a loss through its `logpdf` reaches the margins (#302).
- A compiled `Kde1d` handed to the torch backend is lifted with `TorchKde1d.from_kde1d` rather than refused, so `margins="kde"` behaves like `margins=None` there. The response margin routes through the same seam, so a per-column `margins=` no longer gives the regressor torch covariate margins and a NumPy response (#302).
- `Vinedist` gains an input-coercion seam and takes the array namespace from the columns its margins return rather than from the input, so a distribution whose parts live on one namespace accepts input from another without detaching it (#302).
- Lift the torch path's continuous-only restriction on the *copula* half: `TorchVinecop`'s three constructors take `var_types`, the stored pair copulas stay continuous interpolation grids, and an edge that sees an atom evaluates through `DiscretePair` -- so `state_dict` / `.to()` / pickling still see only real `nn.Module` parameters. `TorchBicop.from_data` accepts the four-column layout and reuses the compiled `find_latent_sample`, which is what `TllBicop::fit` consumes for a discrete edge after [vinecopulib#739](https://github.com/vinecopulib/vinecopulib/pull/739). `TorchVinecopBackend` and `TorchVinedist` therefore accept discrete columns (#302).
- A discrete torch vine refuses two fast paths rather than approximating: the integral cache (`cache_integrals=None` resolves to `False` and an explicit `True` raises, since differencing a bilinearly interpolated `cdf` gives 38% max error on a `("d","d")` density), and the batched cascade, whose stacked per-level grids carry no distribution function for the h-function quotients to difference (#302).
...
- Flip torch defaults based on a bicop + vine benchmark sweep: `cache_integrals=True` everywhere, `batched` resolves device-aware via `_default_batched()` (`True` on cuda, `False` on cpu) (#219). Remeasured at n = 10000 (#305): the cache is worth 165x on `cdf` (cpu) and 23x (cuda) and 4.4x / 1.4x on `hfunc*`; `hinv*` are unchanged at 1.0x, since #305 stopped them reading a cache in either mode (#307 has them read the prefix table again, for the conditional cumulative only). Both `batched` defaults hold -- batched always wins on cuda, and loses on cpu at n = 10000.
...
- The sklearn estimators publish the fitted model as `distribution_`, a `Vinedist` that evaluates through the fitted backend, and the family-selection table of any selecting margin as `selection_report_` (#292).
- `distribution_.sample` goes through the backend that fitted the vine, as its other methods do; the wrapper had kept the pre-rename name, so the call fell through to the raw vine and skipped the backend's conversion of the result (#292).
...
- Document vine distributions: a `concepts.rst` section on the `{pdf, cdf, icdf}` margin contract, why `pdf` is the density with respect to the margin's own reference measure, and what the two-step estimator costs (#292).
- Add `examples/11_vine_distributions.ipynb`, worked on data with known margins so every estimate is checked against a truth (#292).
- Drop `AGENTS.md`'s machine-specific "Running heavy commands" section, keeping only the repo-level half (which CI leg covers what). The file's own rule excludes machine-local guidance, and contributors work from machines without those constraints (#292).
- `inverse_rosenblatt` accepts `batched=True`, grouping the cascade's cells by longest-path level of the `(var, tree)` dependency graph rather than by tree, since its dependencies run across trees; `sample` and `cdf` forward the flag. Bit-identical to the per-edge cascade, and on CUDA `sample` is 3.3-5.9x faster and `cdf` 2.5-5.0x. `batched=True` now downgrades wherever no batched path exists, as `pdf` and `rosenblatt` already did (#307).
- The `[torch]` extra floors at `torch>=2.2`: `nn.Module.__getstate__` arrived in 2.1 and `torch.compiler.cudagraph_mark_step_begin`, which the compiled cascade calls before each graph replay, in 2.2 (#307).
- Add `FitControlsTorchVinecop.compile` and the `TorchVinecop.compile_cascades` property: the batched cascades run through `torch.compile(dynamic=False)`, on CUDA replayed as a CUDA graph. Off by default -- the first call at each input shape pays tens of seconds of Inductor, so it pays for a cascade called repeatedly and not for one evaluation (#307).
- Cut the batched cascade's kernel-launch count, which is what bounds it on CUDA rather than arithmetic: one grid search per tree level shared by the density and both h-functions, linear blends spelled as `lerp`, and a single stacked call for a level's two h-functions. Per tree level, 207 -> 89 launches for `pdf` and 162 -> 68 for `rosenblatt` (#307).

### Build / packaging

- Add `--mode fit` to `scripts/bench_torch_vinecop.py`, timing `TorchVinecop.from_data` against `pv.Vinecop.from_data` across the `select` / fixed-skeleton arms, device, dtype, grid type and a new `--grid-sizes` axis, so the vine-level fit cost recorded ad hoc under #305 can be re-measured on other hardware. `--profile-ace` counts and times the `Tensor.item()` host syncs in `_ace`'s convergence loops. `--dtypes` now rejects a token it does not know instead of silently running float32 under its name (#309).
- Repair `scripts/bench_torch_precision.py`'s vine section, which passed `cache_integrals` / `grid_type` straight to `TorchVinecop.from_data` and so raised `TypeError` on its first cell; those knobs live on `FitControlsTorchVinecop` / `FitControlsTorchBicop` (#292).
...
- Replace `mypy` with `ty` (Astral's type checker, alpha) and enable strict checks against a Python 3.11 baseline; only `pyvinecopulib.pyvinecopulib_ext` is allowed as an unresolved import (#210).
...
- Migrate macOS CI to `macos-15` and re-add `macos-15-intel` (`MACOSX_DEPLOYMENT_TARGET=10.13` for nanobind's aligned `new`/`delete`); wheel matrix is now 5 platforms × 2 ABI (10 wheels).
...
- `DiscretePair` reads a rectangle's probability by differencing four `cdf` values, matching the compiled pair bit for bit. `TorchBicop.rect_mass` is more accurate (1.2e-15 against 9.2e-15 at a `1/8`-wide atom) but is not used: the discrete cascade amplifies a 1e-15 pair-level difference to 8.5e-8 at the vine, which would break the documented torch-to-C++ parity (#306).
- `cache_integrals=True` is no longer refused on a discrete torch vine. It was, because the cache was a bilinear interpolation of tabulated integrals and a discrete edge differenced it -- 38% maximum relative error on a `("d","d")` density. The tables are exact now and the edge reads `rect_mass` instead, so cached and uncached agree to 1e-12 at the pair level (#306).
- `VinecopBase.select(tree_criterion="cxi")` symmetrizes Chatterjee's xi as the compiled selector does, taking the larger of the two directions. It computed one direction only, which selected a different tree ([vinecopulib#754](https://github.com/vinecopulib/vinecopulib/pull/754)).
- `VinecopBase.reorient` and `select(conditioning_set=)` accept a truncated model: the trees above the truncation are independence, so the peel has nothing to move there. At `trunc_lvl == 0` the relabeled order is a stable partition and every set is admissible ([vinecopulib#743](https://github.com/vinecopulib/vinecopulib/pull/743), [vinecopulib#752](https://github.com/vinecopulib/vinecopulib/pull/752)).
- `DiscretePair`'s degenerate branches hold a collapsed argument at the atom's midpoint in **both** terms of the surviving quotient, matching upstream. The second term sat a quarter of an atom away, which is `8.7e-05` on a `4e-5`-wide atom ([vinecopulib#744](https://github.com/vinecopulib/vinecopulib/pull/744)).
- `TorchVinecop.from_vinecop` accepts a compiled vine with an empty pair store, which upstream now allows as independence everywhere at any depth; it tripped the constructor's shape check (#306).
...
- Cut the launch count of a `tll` fit, so a single pair copula is faster on cuda than single-threaded C++ from n = 1000 up (was slower everywhere below n = 10000). The margin renormalization runs its passes on the host when the grid is small -- an `m x m` grid with `m` in the tens made it a few hundred kernel launches to move a few thousand numbers -- `_ace` iterates on contiguous vectors instead of the columns of an `(n, 2)` matrix, and the 2x2 Cholesky inverse and determinant use closed forms rather than `torch.linalg`, which dispatched to cuSOLVER and read its `info` back to the host. Grid factories take `device=` instead of being built on the host and copied. Measured n = 500 / 2000: 15.6 -> 9.8 ms and 22.8 -> 17.0 ms, against C++ at 8.2 / 33.4; the fitted grid moves by at most 1e-13, where the C++ parity gate is 1e-11 (#305).
- `TorchMargin` follows `.to()`. A factory that closes over its own parameters registers none, so the construction-time placement is the only record of where the margin lives -- and `_apply` never updated it, leaving such a margin drawing its samples on the cpu after a move to cuda. The shared `_trim` helper reaches `Vinedist`'s clamps too, which were the last copy of the `1 - 1e-10` literal (#292).
- Correct `_resolve_cache_integrals`' rationale for accepting a discrete vine. It said the edge reads its rectangle probability through `rect_mass` rather than by differencing; the `rect_mass` consumption was reverted in this branch to keep the torch-to-C++ cascade bit-identical, so the edge still differences. The conclusion is unchanged -- the tables are exact, so differencing them is no longer lossy (#306).
...
- `TorchBicop.sample(seeds=...)` no longer calls `torch.manual_seed`, which reseeded the global CPU generator and every CUDA one as a side effect of seeding a single pair copula. It uses a device-local `torch.Generator`, matching the vine's own `_sample_uniform` hook (#305).
...
- Fit the `{0, 1}` dummies of an expanded unordered categorical on `[0, 1]` in the sklearn estimators. Their bounds are known exactly, but were never passed, so the kernel-density grid was padded past the data and put mass on values that cannot occur (#292).
- Pass the declared support of an ordered categorical to its marginal fit in the sklearn estimators. Only the expanded `{0, 1}` dummies were bounded, so a count column's kernel-density grid was still padded below its smallest level (#292).
...
- A `TorchVinecop` whose fit chose `indep` on some edges and `tll` on others could not be evaluated with `batched=True`: an independence pair carries a two-point sentinel grid, so stacking a tree level raised from `torch.stack`. The bake now substitutes an independence density built on the shared grid. Present since the torch module landed, and reached by default on CUDA (#307).
- Pickling a `TorchVinecop` after evaluating it raised `TypeError: cannot pickle 'module' object`; the array-namespace memo is dropped from the state and re-resolved on demand (#307).
- `sample`, `cdf` and `inverse_rosenblatt` evaluate under `no_grad`, so a stacked bake one of them triggered was detached and every later `pdf(batched=True)` lost its gradient. The bake records whether it was made under grad and is redone once if not (#307).
...
- Every discrete or mixed fit with a nonparametric (`tll`) pair copula changes: the fit now uses its latent sample rather than discarding it, and evaluation computes a real discrete density instead of the continuous one at the atom midpoint ([vinecopulib#739](https://github.com/vinecopulib/vinecopulib/pull/739), [vinecopulib#736](https://github.com/vinecopulib/vinecopulib/issues/736)). `tll` is the default family, so this was the default discrete path: grids, densities, h-functions, `loglik`, `aic` / `bic` / `mbicv` all move, and family and structure selection with them. Continuous fits are untouched.
- Atom masses of a `tll` pair now sum to one exactly, where the midpoint rule was off by up to 10% and did not converge away with more atoms ([vinecopulib#739](https://github.com/vinecopulib/vinecopulib/pull/739)). On a three-variable discrete vine `Vinecop.loglik` was 522.11 against a correct 515.56.
...
- `Vinecop.select` reports the threshold it chose on the "Select threshold" line of `show_trace` output, which named the criterion value instead ([vinecopulib#735](https://github.com/vinecopulib/vinecopulib/pull/735)).
...
- `TorchKde1d.icdf` is pinned to the compiled quantile by a tolerance rather than an equality. Since [kde1d#33](https://github.com/vinecopulib/kde1d-cpp/pull/33) that quantile is a Newton iteration, and the compiled result is not portable to the last bits -- rebuilding kde1d with `-march=native` and nothing else moves it 19 ULPs -- so no port can equal every build of it. The pin is two orders tighter than that variation and still fails loudly on an algorithmic divergence (#312).
- Bump `lib/kde1d` to `main`, past eight pull requests. Numbers move for every fit: `linbin` no longer drops the observation at the top of its binning range, so automatic bandwidths shift by O(1/n) ([kde1d#29](https://github.com/vinecopulib/kde1d-cpp/pull/29)), and continuous quantiles are inverted per cell rather than by 35 global bisections ([kde1d#33](https://github.com/vinecopulib/kde1d-cpp/pull/33)) (#312).
- A discrete `Kde1d`'s `xmin` / `xmax` now mean its **integer support**, not grid endpoints: bounds and data must be integers, the fitted grid runs from `xmin - 0.5` to `xmax + 0.5` where the jitter cells end, and the masses are normalized over the declared support. Previously the bounds were nearly inert for a discrete fit and the grid was snapped onto them, truncating the outermost half-cells — which is what every ordered-categorical margin the sklearn estimators fit was doing ([kde1d#37](https://github.com/vinecopulib/kde1d-cpp/pull/37)) (#312).
- Add `Kde1d(boundary_repair=True)`, which lets a finite bound be fitted with a local-linear boundary estimator rather than the transformed fit. Eligibility rather than a guarantee: it needs a finite bound and at least sixteen observations, and does nothing at all when neither bound is set ([kde1d#36](https://github.com/vinecopulib/kde1d-cpp/pull/36)) (#312).
- Add `Kde1d.actual_grid_size`, the number of grid points a fit built, which is one more than the `grid_size` requested (#312).
- Rewrite the public `//!` comments on `Kde1d`, which this package lifts verbatim into its Python API documentation: `get_actual_grid_size` named a cause that no longer exists, `quantile` documented a domain its own guard contradicts, `str()` had no comment at all, and both grid constructors documented a `prob0` they declared as `prob0_` ([kde1d#38](https://github.com/vinecopulib/kde1d-cpp/pull/38)) (#312).
- Move the `lib/vinecopulib` pin from the branch of [vinecopulib#754](https://github.com/vinecopulib/vinecopulib/pull/754) to `main`, which carries it squashed plus the bounds-checked structure accessors of [vinecopulib#756](https://github.com/vinecopulib/vinecopulib/pull/756), and bump `lib/wdm` to its `v0.3.0` tag (#312).
...
- Re-select the bandwidth when an estimator is refitted: the requested value is now kept apart from the value a fit settled on, so `fit` on new data no longer reuses the previous fit's bandwidth. Grid-built densities also get defined `multiplier` / `bandwidth` / `degree` instead of indeterminate ones ([kde1d#28](https://github.com/vinecopulib/kde1d-cpp/pull/28)).
