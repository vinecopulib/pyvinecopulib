...
- `VineForestDensity` / `VineForestRegressor` expose selection diagnostics as fitted attributes: `mcs_size_`, `default_rank_val_`, and `random_logliks_val_` are always set from the validation-side quantities the survivor-selection step already computes; a new `compute_train_scores=False` `__init__` parameter additionally scores every candidate (and the Dissmann default) on the full training portion and stores `default_rank_train_` / `random_logliks_train_`. Also fixes the degenerate no-survivor path, which previously left the private `_default_loglik` / `_default_estimator` unset (#247).
