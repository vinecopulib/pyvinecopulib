...
- Add `pyvinecopulib.core.DiscretePair`, which evaluates a *continuous* pair copula on a discrete or mixed edge from its `pdf` / `cdf` / `hfunc1` / `hfunc2` alone. `BicopLike` stays a two-column continuous contract, so a custom pair copula gains discrete support by implementing `cdf`; the output matches `Bicop`'s own discrete surface bit-for-bit at rotation 0 for the parametric families. A nonparametric (`tll`) pair differs, because `Bicop` returns the continuous density at the atom midpoint there ([vinecopulib#739](https://github.com/vinecopulib/vinecopulib/pull/739)) (#301).
