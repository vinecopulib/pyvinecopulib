...
- Drop `AGENTS.md`'s machine-specific "Running heavy commands" section, keeping only the repo-level half (which CI leg covers what).
...
- Cut the batched cascade's kernel-launch count, which is what bounds it on CUDA rather than arithmetic: one grid search per tree level shared by the density and both h-functions, linear blends spelled as `lerp`, and a single stacked call for a level's two h-functions. Per tree level, 207 -> 89 launches for `pdf` and 162 -> 68 for `rosenblatt` (#307). The file's own rule excludes machine-local guidance, and contributors work from machines without those constraints (#292).
