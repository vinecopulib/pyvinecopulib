...
- Resolve a one-dimensional `x` by naming the reading rather than guessing it: `covariate_row(x)` reads `(p,)` as one covariate row shared across observations, `covariate_column(x, n)` reads `(n,)` as one covariate per observation. `prepare_covariates` still refuses `(n,)` -- the shape says nothing about which axis is which -- and its message now names both (#334).
    - `covariate_column` requires `n` and checks the values against it, since an `x` of the wrong length silently reshaped would pair each observation with another's covariate
