...
class MarginBase(MarginLike[ArrayT], ABC):
  ...
  ``loglik``, ``simulate`` and ``__repr__``. Override any of them with a
  ...
  as :class:`pyvinecopulib.utils.Kde1d` and the scikit-learn estimators do. A
  ...
  def simulate(self, n: int, *, seeds: Optional[list[int]] = None) -> ArrayT:
    ...
    """
    base = self._simulate_uniform(n, list(seeds) if seeds else [])
    return self.icdf(base)

  def _simulate_uniform(self, n: int, seeds: list[int]) -> ArrayT:
    ...
    NotImplementedError
        ...
        :func:`pyvinecopulib.utils.simulate_uniform` free function.
    """
    raise NotImplementedError(
      f"{type(self).__name__}._simulate_uniform is not defined; override it "
      "with the array namespace's RNG to enable simulate()."
    )
