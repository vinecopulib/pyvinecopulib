...
class MarginSelector(MarginBase[np.ndarray]):
  ...
  pyvinecopulib.margins.Kde1dMargin : The fallback, and the library default.
  ...
  **Nothing is skipped silently.** A candidate that fails to fit, or that fits
  ...
  :class:`Kde1dMargin` and warns once, naming the variable and the reasons —
  ...
  **Observation weights are not supported**, because the parametric candidates
  ...
  def selected_(self) -> Any:
    ...
    MarginLike
        ...
        :class:`Kde1dMargin` fallback when every candidate was rejected.
  ...
  def _fallback(
    ...
    margin = Kde1dMargin(type="discrete" if counts else "continuous")
    margin.fit(data)
  ...
  def simulate(
    ...
    """
    return np.asarray(self.selected_.simulate(n, seeds=seeds), dtype=float)
