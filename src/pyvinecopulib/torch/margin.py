...
class TorchMargin(MarginBase[Tensor], torch.nn.Module):
  ...
  **Coverage.** ``cdf`` is what puts a variable on the copula scale, so a family
  ...
  - neither: ``Beta``, ``StudentT``, ``FisherSnedecor``, ``VonMises``. No
    ...
    :class:`pyvinecopulib.margins.Kde1dMargin`, or a SciPy distribution through
  ...
  **Constrained parameters.** Values are stored on the family's natural scale,
    ...
    *,
    ...
    if not _implements(probe, "cdf"):
      raise NotImplementedError(
        ...
        "differenced around that. Use pyvinecopulib.margins.Kde1dMargin, or a "
  ...
  def pdf(self, x: Tensor) -> Tensor:
    ...
    Tensor, shape (n,), dtype float
        Density values.
    """
    return self.distribution.log_prob(x).exp()
  ...
  def icdf(self, p: Tensor) -> Tensor:
    ...
    :meth:`pyvinecopulib.torch.TorchVinecop.simulate` does: the derivative of a
  ...
  def _simulate_uniform(self, n: int, seeds: list[int]) -> Tensor:
