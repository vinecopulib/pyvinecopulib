...
``0``, ``90``, ``180``, ``270``), a matrix of parameters, and variable
types (two strings, one for each variable, either ``"c"`` for
...
|---------------|-----------------------|-----------------------| | -
| Independence | "indep" | | Elliptical | Gaussian | "gaussian" | | "
| Student t | "student" | | Archimedean | Clayton | "clayton" | | " |
Gumbel | "gumbel" | | " | Frank | "frank" | | " | Joe | "joe" | | " |
Clayton-Gumbel (BB1) | "bb1" | | " | Joe-Gumbel (BB6) | "bb6" | | " |
Joe-Clayton (BB7) | "bb7" | | " | Joe-Frank (BB8) | "bb8" | |
Extreme-Value | Tawn | "tawn" | | Nonparametric | Transformation
kernel | "tll" |)""";
      // Symbol: vinecopulib::Bicop::Bicop
      struct /* ctor */ {
        // Source: vinecopulib/bicop/class.hpp:57
        const char* doc_copy =
...
----------
input :
    The ``nlohmann::json`` object to convert from (see ``to_json()``
    for the structure of the input).)""";
      } ctor;
...
``Bicop::loglik()``) and :math:`p` is the (effective) number of
parameters of the model. The AIC is a consistent model selection
criterion even for nonparametric models.
...
where :math:`\\mathrm{loglik}` is the log-liklihood (see
``Bicop::loglik()``) and :math:`p` is the (effective) number of
parameters of the model. The BIC is a consistent model selection
criterion for parametric models.
...
required for ``u``: the first \\eqn{n \\times 2} block contains
realizations of \\eqn{F_{X_1}(x_1), F_{X_2}(x_2)}. The second \\eqn{n
\\times 2} block contains realizations of \\eqn{F_{X_1}(x_1^-),
F_{X_1}(x_1^-)}. The minus indicates a left-sided limit of the cdf.
For, e.g., an integer-valued variable, it holds \\eqn{F_{X_1}(x_1^-) =
F_{X_1}(x_1 - 1)}. For continuous variables the left limit and the cdf
itself coincide. Respective columns can be omitted in the second
block.
...
allowd).)""";
      ...
      struct /* check_var_types */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:1069
        const char* doc =
...
"d".)""";
      ...
      struct /* check_weights_size */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:1048
        const char* doc =
R"""(Checks whether weights and data have matching sizes.)""";
      ...
      struct /* fit */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:827
        const char* doc =
...
R"""(Adjusts the copula model to a change in the variable order.)""";
      ...
      struct /* format_data */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:955
        const char* doc =
...
models only require two columns, discrete models always four))""";
      ...
      struct /* get_bic */ {
        // Source: vinecopulib/bicop/class.hpp:86
        const char* doc = R"""(Gets the bic (only for fitted objects).)""";
      } get_bic;
      // Symbol: vinecopulib::Bicop::get_bicop
      struct /* get_bicop */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:786
        const char* doc = R"""()""";
      } get_bicop;
      // Symbol: vinecopulib::Bicop::get_family
      struct /* get_family */ {
        // Source: vinecopulib/bicop/class.hpp:71
...
the sense that it can be used in the calculation of fit statistics.)""";
      ...
      struct /* get_parameters_lower_bounds */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:771
        const char* doc =
R"""(Gets lower bounds for copula parameters.)""";
      ...
      struct /* get_parameters_upper_bounds */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:778
        const char* doc =
...
Returns
-------
A length n vector of the inverse of the first h-function evaluated
    at ``u``.)""";
      } hinv1;
...
Returns
-------
A length n vector of the inverse of the second h-function
    evaluated at ``u``.)""";
      } hinv2;
...
where :math:`\\mathrm{loglik}` is the \\log-liklihood (see
``Bicop::loglik()``), :math:`p` is the (effective) number of
parameters of the model, and :math:`\\psi_0` is the prior probability
of having a non-independence copula and :math:`I` is an indicator for
the family being non-independence.
...
A length n vector of copula densities evaluated at ``u``.)""";
      ...
      struct /* prep_for_abstract */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:1020
        const char* doc =
...
0deg-rotation).)""";
      ...
      struct /* rotate_data */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:984
        const char* doc =
...
u :
    An ``n x 2`` matrix.)""";
      ...
      struct /* select */ {
        // Source: vinecopulib/bicop/implementation/class.ipp:872
        const char* doc =
R"""(Selects the best fitting model.

The function calls ``Bicop::fit()`` for all families in ``family_set``
and selecting the best fitting model by either BIC or AIC, see
``Bicop::bic()`` and ``Bicop::aic()``.
...
An :math:`n \\times 2` matrix of samples from the copula model.)""";
      ...
      struct /* str */ {
        // Source: vinecopulib/bicop/class.hpp:133
        const char* doc =
R"""(Summarizes the model into a string (can be used for printing).)""";
      ...
      struct /* tau_to_parameters */ {
        ...
        const char* doc =
R"""(Converts a Kendall's :math:`\\tau` into copula parameters for
...
R"""(A shared pointer to an object of class AbstracBicop.)""";
    ...
    struct /* CVineStructure */ {
      // Source: vinecopulib/vinecop/rvine_structure.hpp:200
      const char* doc =
...
trunc_lvl :
    ...
    struct /* DVineStructure */ {
      // Source: vinecopulib/vinecop/rvine_structure.hpp:183
      const char* doc =
...
psi0 :
    Only for ``selection_criterion = "mbic"``, the prior probability
    of non-independence.
...
select_families :
    Whether the families shall be selected automatically, or should
    the method simply update the parameters for the pair copulas
...
mst_algorithm :
    The algorithm for building the maximum spanning tree (``"prim"``
    or ``"kruskal"``) during the tree-wise structure selection.)""";
...
mst_algorithm :
    The algorithm for building the maximum spanning tree (`"prim"`` or
    ``"kruskal"``) during the tree-wise structure selection.)""";
      } ctor;
...
R"""(Summarizes the controls into a string (can be used for printing).)""";
    ...
    struct /* RVineStructure */ {
      // Source: vinecopulib/vinecop/rvine_structure.hpp:89
      const char* doc =
...
representations of truncated models. For instance, the 2-truncated
model is represented by the same diagonal and the following truncated
triangular array:
...
R"""(Instantiates from the variable order (diagonal elements of the R-vine
array) and a triangular structure array (elements above the diagonal).
...
Implementation of Algorithm 13 in Harry Joe's 2014 book (p. 288), but
there's a typo: the end of line 6 in the book should be 'column j'
instead of 'column k'.
...
natural_order :
    Whether indices correspond to natural order.)""";
      ...
      struct /* struct_array_ */ {
        // Source: vinecopulib/vinecop/rvine_structure.hpp:164
        const char* doc = R"""()""";
      } struct_array_;
...
mat :
    A array representing the R-vine array.)""";
      ...
      struct /* trunc_lvl_ */ {
        // Source: vinecopulib/vinecop/rvine_structure.hpp:163
        const char* doc = R"""()""";
      } trunc_lvl_;
...
While a structure of dimension ``d`` contains at most ``d-1`` nested
levels, this function extracts a sub-structure based on a given
truncation level.
...
``RVineStructure`` objects) and the pair-copulas (see ``Bicop``
objects).)""";
...
pair_copulas :
    ``Bicop`` objects specifying the pair-copulas, namely a nested
    list such that ``pc_store[t][e]`` contains a ``Bicop`` object for
...
where :math:`\\mathrm{loglik}` is the log-liklihood (see
``Vinecop::loglik()``) and :math:`p` is the (effective) number of
parameters of the model. The AIC is a consistent model selection
criterion even for nonparametric models.
...
u :
    ...
    variables (see ``select()`` or ``Vinecop::pdf()``).
...
``Vinecop::loglik()``) and :math:`p` is the (effective) number of
parameters of the model. The BIC is a consistent model selection
criterion for nonparametric models.
...
u :
    An :math:`n \\times (d + k)` or :math:`n \\times 2d` matrix of
    evaluation points, where :math:`k` is the number of discrete
    variables (see ``Vinecop::select()`` or ``Vinecop::pdf()``).
...
When at least one variable is discrete, two types of "observations"
are required in ``u``: the first :math:`n \\; x \\; d` block contains
realizations of :math:`F_{X_j}(X_j)`. The second :math:`n \\; x \\; d`
block contains realizations of :math:`F_{X_j}(X_j^-)`. The minus
indicates a left-sided limit of the cdf. For, e.g., an integer-valued
variable, it holds :math:`F_{X_j}(X_j^-) = F_{X_j}(X_j - 1)`. For
...
Returns
-------
A vector of length ``n`` containing the copula distribution
    values.)""";
      } cdf;
...
This method fits the pair-copulas of a vine copula model. It is
assumed that the structure and pair-copula families are already set.
The method is equivalent to calling ``fit()`` for each pair-copula in
the model. The same can be achieved by calling ``select()`` with the
same data and a ``FitControlsVinecop`` object instantiated with
...
a nested std::vector with entry ``[t][e]`` corresponding to edge
    ``e`` in tree ``t``.)""";
      ...
      struct /* get_all_pair_copulas */ {
        // Source: vinecopulib/vinecop/implementation/class.ipp:518
        const char* doc =
...
a nested std::vector with entry ``[t][e]`` corresponding to edge
    ``e`` in tree ``t``.)""";
      ...
      struct /* get_all_rotations */ {
        // Source: vinecopulib/vinecop/implementation/class.ipp:574
        const char* doc =
...
a nested std::vector with entry ``[t][e]`` corresponding to edge
    ``e`` in tree ``t``.)""";
      ...
      struct /* get_all_taus */ {
        // Source: vinecopulib/vinecop/implementation/class.ipp:644
        const char* doc =
...
``Vinecop::inverse_rosenblatt()`` computes
...
If some variables have atoms, Brockwell (10.1016/j.spl.2007.02.008)
proposed a simple randomization scheme to ensure that output is still
independent uniform if the model is correct. The transformation reads

.. math:: U_{M[d - j, j]}= W_{d - j} F(V_{M[d - j, j]} | V_{M[d - j -
1, j - 1]}, \\dots, V_{M[0, 0]}) + (1 - W_{d - j}) F^-(V_{M[d - j, j]}
| V_{M[d - j - 1, j - 1]}, \\dots, V_{M[0, 0]}),

where :math:`F^-` is the left limit of the conditional cdf and
:math:`W_1, \\dots, W_d` are are independent standard uniform random
variables. This is used by default. If you are interested in the
conditional probabilities

.. math:: F(V_{M[d - j, j]} | V_{M[d - j - 1, j - 1]}, \\dots, V_{M[0,
0]}),
...
seeds :
    Seeds to scramble the quasi-random numbers; if empty (default),
    the random number quasi-generator is seeded randomly. Only
    relevant if there are discrete variables and ``randomize_discrete
    = TRUE``.
...
An :math:`n \\times d` matrix of independent uniform variates.)""";
      ...
      struct /* select */ {
        ...
        const char* doc =
R"""(In other words, ``select()`` behaves differently depending on its
current truncation level and the truncation level specified in the
controls, respectively called ``trunc_lvl`` and ``controls.trunc_lvl``
in what follows. Essentially, ``controls.trunc_lvl`` defines the
object's truncation level after calling ``select()``:
...
tau) is corrected for ties (see the `wdm
<https://github.com/tnagler/wdm>`_ library).

If the ``controls`` object has been instantiated with
``select_families = false``, then the method simply updates the
parameters of the pair-copulas without selecting the families or the
structure. In this case, this is equivalent to calling ``fit()`` for
each pair-copula, albeit potentially in parallel if ``num_threads >
1``.
...
Sampling from a vine copula model is done by first generating :math:`n
\\times d` uniform random numbers and then applying the inverse
...
``"pair copulas"``. ``"pair copulas"`` contains a list of attributes
for the trees (``"tree1"``, ``"tree2"``, etc), each containing a list
of attributes for the edges (``"pc1"``, ``"pc2"``, etc). See
``Bicop::to_file()`` objects for the encoding of pair-copulas.
...
While model for a ``d`` dimensional random vector contains at most
``d-1`` nested trees, this function extracts a sub-model based on a
