...
def extract_comment(cursor, deprecations):
  ...
  for d in possible_d:
    if all([
        d.extent.start.line >= c.extent.start.line, d.extent.start.line <=
        (c.extent.start.line + 5), d.extent.end.line <= c.extent.end.line
    ]):
      found = d
...
def process_comment(comment):
  ...
  for cmd_ in [
    ...
    s = re.sub(r'[@\\]%s\s+[\w:.]+(\s+".*?")?(\s+[\w:.]+=[\w:.]+)?s+' % cmd_,
               r'', s)
...
def choose_doc_var_names(symbols):
  ...
  overload_arg_types = [[t.spelling for t in s.cursor.type.argument_types()]
                        for s in symbols]
  overload_arg_names = [[a.spelling for a in s.cursor.get_arguments()]
                        for s in symbols]
...
def print_symbols(f, name, node, level=0):
  ...
  for symbol, doc_var in zip(symbol_iter, doc_vars):
    ...
    iprint('  const char* {} ={}R"""({})""";'.format(doc_var, delim,
                                                     comment.strip()))
