...
def _bounded_kde(column: np.ndarray) -> Kde1dMargin:
  """A margin bounded by the observed range, so its own grid is non-uniform."""
  column = np.asarray(column, dtype=float)
  return Kde1dMargin(xmin=float(column.min()), xmax=float(column.max())).fit(
    column
  )
...
def test_a_bounded_response_margin_is_as_accurate_as_an_unbounded_one() -> None:
  ...
  A bounded ``Kde1dMargin`` grids its own support non-uniformly, which is what
