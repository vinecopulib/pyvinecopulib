...
def test_simulate_returns_data_scale_samples(dist: TorchVinedist) -> None:
  ...
  first = dist.simulate(200, seeds=[7])
  assert first.shape == (200, 3)
  torch.testing.assert_close(dist.simulate(200, seeds=[7]), first)
...
def test_from_data_stays_on_the_numpy_lane(
  ...
  It would fit `Kde1dMargin` margins and a compiled copula, neither of which
