...
def test_a_factory_registering_no_parameters_still_evaluates() -> None:
  ...
  draws = margin.simulate(5, seeds=[3])
...
def test_simulate_is_seeded_and_inverts_the_cdf() -> None:
  ...
  margin = _normal(0.3, 1.4)
  first = margin.simulate(64, seeds=[7])
  torch.testing.assert_close(margin.simulate(64, seeds=[7]), first)
  assert not torch.allclose(margin.simulate(64, seeds=[8]), first)
