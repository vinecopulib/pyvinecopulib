...
def test_guard_explains_missing_features(
...
def test_guard_fails_open_when_the_cpu_cannot_be_probed(
  monkeypatch: pytest.MonkeyPatch,
) -> None:
  monkeypatch.setattr(_cpu.platform, "machine", lambda: "x86_64")
  monkeypatch.setattr(_cpu, "_requires_x86_64_v3", lambda: True)
...
def test_guard_ignores_other_architectures(
  monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_linux_probe_reports_none_without_proc(
  monkeypatch: pytest.MonkeyPatch,
) -> None:
