...
from pyvinecopulib.margins import Kde1dMargin
...
class _ListVinecop(VinecopBase[Any]):
  ...
  expected to do. ``_simulate_uniform`` matches the C++ draw so the randomized
  ...
  def _simulate_uniform(
    ...
    return pv.utils.simulate_uniform(n, self.d, qrng, seeds)
...
def test_randomized_rosenblatt_matches_vinecop(
  ...
  # The default path. Same seeds through the same `simulate_uniform`, so the
...
def test_vinedist_with_a_discrete_margin_uses_the_cascade() -> None:
  ...
  margins = [Kde1dMargin(type="discrete", xmin=0.0).fit(counts)] + [
    Kde1dMargin().fit(x[:, j]) for j in range(1, _D)
  ]
